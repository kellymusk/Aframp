    ...
    useEffect(() => {
        const timer = setInterval(() => {
            ...
            if (distance < 0) {
                ...
                // Assuming 15 mins total for progress bar calculation
            ...
            <div className={cn(
                ...
                <div className="flex items-start gap-4">
                    <div className={cn(
                        ...
                        isExpired ? "bg-destructive/10 text-destructive" : "bg-accent/10 text-accent-foreground"
                    )}>
                        <Clock className="h-5 w-5" />
                    </div>
                    <div className="space-y-2 flex-1">
                        <div className="flex items-center justify-between">
                            <h4 className={cn("font-bold tracking-tight", isExpired ? "text-destructive" : "text-accent")}>
                                {isExpired ? "Rate Expired" : "Rate locked until: 14:45"}
                            </h4>
                            {!isExpired && <span className="text-xs font-bold text-accent/80">(10 mins remaining)</span>}
                        </div>
                        <p className={cn("text-sm font-medium leading-relaxed", isExpired ? "text-destructive/80" : "text-foreground/70")}>
                            {isExpired
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                <div className="flex items-center gap-4 p-4 bg-primary/5 rounded-2xl border border-primary/10 group hover:bg-primary/10 transition-colors">
                    ...
                    <div className="text-sm">
                        ...
                        <p className="text-foreground/70 text-xs font-medium">You'll receive SMS when complete</p>
                    </div>
                </div>
